...
public class Tutorial_Camera_3 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        THCBscript = THCB.GetComponent<Tutorials_HandControllerButton>();
    ...
    {
        // ストックから物を取るときの自動選択
        if (!THCBscript.HoldingFlg)
        {
            if (stock_flg && AutSelect_flg){
                if (THCBscript.TargetTag != "Item")
                {// その場に取れるものがなかった時
    ...
    void DownKeyCheck()
    {
        ...
        else if (Input.GetKeyDown(KeyCode.DownArrow) || (0 > Input.GetAxisRaw("XBox_Pad_V") && !button_flg))
        {
             ...
             * ・盛り付け場を見ているときに下を押した時
             *　　ゴミ箱を向く  */
            if (cursor == 15 || (THCBscript.HoldingFlg && ClickObj.transform.GetChild(0).name == "ItemKoge"))
            {
                cursor = 22;
                gomi_flg = true;
            }
            else if (cursor == 14 || (THCBscript.HoldingFlg && ClickObj.transform.GetChild(0).name == "ItemKoge"))
            {
                cursor = 23;
                gomi_flg = true;
            }
    ...
    void PotSelect()
    {
        ...
        if (THCBscript.ItemPowder && THCBscript.TargetTag == "Item")
        {
            ...
            MoveLight();
                ...
                !THCBscript.HoldingFlg && THCBscript.TargetTag != "Item")
        {
            Pcursor += 1;
