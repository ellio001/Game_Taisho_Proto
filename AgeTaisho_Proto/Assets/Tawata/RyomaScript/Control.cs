...
public class Control : MonoBehaviour {
    ...
    public GameObject HCBR;
    HandControllerButton_Ryoma HCBRscript;
    ...
    void Start() {
        ...
        HCBRscript = HCBR.GetComponent<HandControllerButton_Ryoma>();
    ...
    void Update() {
        ...
        else if (!HCBRscript.MoveFlg) // スペースを離しているかの判定
        {
            if (pot_flg == false) {
            ...
            //if (old_direction != HCBRscript.direction)
            //    HCBRscript.TargetObj.GetComponent<Outline>().enabled = true;
            //else
            //    HCBRscript.TargetObj.GetComponent<Outline>().enabled = false;
    ...
    void PotSelect() {
        ...
        if (HCBRscript.ItemPowder && HCBRscript.TargetTag == "Item") {
            if (HCBRscript.TargetTag != "Item")// おける場所が見つかった時
            {
                tmp_Pcursor = -1;
                potLight_Flg = true;
            }
    ...
    void MoveCamera() {// 三方向にカメラを固定する処理
