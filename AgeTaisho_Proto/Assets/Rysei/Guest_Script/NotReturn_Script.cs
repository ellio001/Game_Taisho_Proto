...
public class NotReturn_Script : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (GuestNowPosition.z >= -2.1 && Order == false)  //席に着いたら処理
        {
            ...
            SideItems[0] = Instantiate(OrderItems[0], DisplayPosition[MyNumber] + new Vector3(0,0.5f,0), transform.rotation);  //客生成(客番号,座標,回転)
            SideItems[1] = Instantiate(OrderItems[0], DisplayPosition[MyNumber + 3] + new Vector3(0, 0.5f, 0), transform.rotation);  //客生成(客番号,座標,回転)
